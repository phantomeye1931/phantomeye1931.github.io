...
function pawnMoves(pos, dir) {
  ...
  if (hasEnemyPiece(pos.inFront.left)) markValid(pos.inFront.left)
  if (hasEnemyPiece(pos.inFront.right)) markValid(pos.inFront.right)
}
```
...
> **Implementation note:** three things worth knowing before you build this.
>
> First, "skip it and continue the trace" can't be unconditional. It only makes sense the *first* time the ray hits anything - if some unrelated piece was already sitting between the attacker and the En-Passantable pawn, that piece is a completely ordinary blocker with no relationship to En Passant at all, and letting the ray see through the En-Passantable pawn anyway would falsely flag that unrelated piece as pinned. I only let this passthrough happen when nothing has been hit yet on this ray.
>
> Second, the marking needs to sit on the pawn's own square, not the skipped square. My original "mark said pawn as En-Passantable" step actually marked the skipped square (needed so the capture-destination check can find it, as noted above) - but pin-tracing walks through actual pieces, and the pawn is standing on its landing square, not the empty square behind it. Marking only the skipped square meant the pin-tracer never recognized the pawn as En-Passantable at all, and just treated it as an ordinary blocker. I ended up marking both squares - the skipped one for capture validation, the pawn's own square for this.
>
> Third, don't reuse the regular 'pinned' marking for this. A genuinely pinned piece may only move along the line to its king; a pseudo-pinned piece may do *anything except* capture En Passant - reusing 'pinned' would incorrectly restrict all its other moves too. This needs its own marking, checked only at the point where an En Passant capture is being considered, not wired into the general move-restriction path pinned pieces go through.
...
- After a move is made, we do the following:
    1. Make sure to forget how we marked squares/pieces last round.
    2. Compute what squares the last moved player is attacking.
    3. Count how many valid moves the enemy of last moved player can make.
    4. Determine if one of the players is in Checkmate or Stalemate.
    5. Switch whose turn it is.
...
If this blog was any use to you, or you have any thoughts, please contact me! I would love to hear about your projects.


<Wrapper classes="right" style="display:flex;justify-content: center;margin-top:4rem">
    <BusinessCard />
</Wrapper>