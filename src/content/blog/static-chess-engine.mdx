...
> **Implementation note:** this wasn't handled automatically for me. My check-evasion filter looks for a check-resolving mark on a move's *destination*, but an En Passant capture lands on the skipped square, one rank from the checking pawn - so nothing marks it. Fix: when marking a pawn as a check origin, also mark the square *behind* it as a line-of-check square.
...
**If the very first thing a ray hits is an En-Passantable piece, we can treat it as if it wasn't there and keep tracing** - it's still a real, currently-standing piece, so nothing past it counts as attacked yet, but it's one that might vanish next move. If we then hit an enemy piece, and the king beyond that, **we find ourselves in our edge case**.

We can then mark that enemy piece as **'En Passant pseudo-pinned'.**
...
> **Implementation note:** the passthrough only makes sense the *first* time the ray hits anything - if some unrelated piece was already sitting between the attacker and the En-Passantable pawn, letting the ray see through it anyway would falsely flag that unrelated piece as pinned. And "seeing through" only means the ray keeps *looking* past it for a pin candidate, not that anything beyond it is already attacked - the pawn hasn't actually been captured yet.
...
function traceSpots(pos, dir) {
  ...
  // An En-Passantable pawn still blocks right now - track that separately from firstHit,
  // or everything past it would falsely count as attacked until something else stops the ray
  alreadyBlocked = blocked
  if (hasPiece(pos)) blocked = true
  ...
  if (!alreadyBlocked || (foundKing && phase == ATTACK)) validateSpot(pos) // Move or capture
